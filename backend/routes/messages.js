...
// Send voice message
router.post('/send-voice/:recipientId', protect, upload.single('voiceMessage'), async (req, res) => {
  try {
    const recipientId = req.params.recipientId;
    const { duration } = req.body;

    if (!req.file || !recipientId) {
      return res.status(400).json({ message: 'Voice file and recipient required' });
    }
    ...
    // Create notification for voice message
    const notification = new Notification({
      recipient: recipientId,
      sender: req.userId,
      type: 'message',
...
router.get('/', protect, async (req, res) => {
  try {
    ...
    messages.forEach(msg => {
      ...
      const lastMessageText = msg.messageType === 'voice' ? '🎙️ Voice message' : msg.text;
